...
def main():
  ...
  for CamIdx in range(0, num_of_cams):
    FFMPEG_Args = ['ffmpeg', "-i", f"1_renderedScene/texture/{start_frame:04d}-{end_frame:04d}.{CamIdx:03d}.mp4", "-ss", "00:00:00", "-vf",  "scale=in_range=limited:out_range=full",  "-f",  "rawvideo", "-pix_fmt", "yuv420p", "-vframes",  "1", f"1_renderedScene/texture/v{CamIdx:02d}_background_{width}x{height}_yuv420p.yuv"]
    print(FFMPEG_Args)
